...
$order = $db->GetOne("SELECT item_order FROM ".cms_db_prefix()."module_cgblog_fielddefs WHERE id = ?", array($fdid));
$time = $db->DBTimeStamp(time());

if ($params['dir'] == "down")
  {
    $query = 'UPDATE '.cms_db_prefix().'module_cgblog_fielddefs SET item_order = (item_order - 1), modified_date = '.$time.' WHERE item_order = ?';
    $db->Execute($query, array($order + 1));

    $query = 'UPDATE '.cms_db_prefix().'module_cgblog_fielddefs SET item_order = (item_order + 1), modified_date = '.$time.' WHERE id = ?';
