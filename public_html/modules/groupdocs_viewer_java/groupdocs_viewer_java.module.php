<?php
#-------------------------------------------------------------------------
# Module: groupdocs_viewer_java - GroupDocs Word, Excel, Powerpoint, PDF Viewer Embedder lets you embed several types of files into your page.
...
#-------------------------------------------------------------------------
class groupdocs_viewer_java extends CMSModule
{

	function GetName()
	{
		return 'groupdocs_viewer_java';
	}
	...
	/*---------------------------------------------------------
	  ...
	  ---------------------------------------------------------*/
	function CheckAccess($perm = 'ModifyGrouDocsViewerJava')
		{
	...
	function get_docs_list($where=array(),$admin=false,$id="",$returnid="",$order=false,$limit=0,$customwhere=false,$customvalues=array(),$customorder=false){
		...
		if(!$order)	$order = "";
		$db =& $this->GetDb();
		$fields = array("width","height","server_url","name");
		
		$query = "SELECT * FROM ".cms_db_prefix()."module_groupdocs_viewer_java_docs A;";
		...
		while ($dbresult && $row = $dbresult->FetchRow()){
			...
			$item->insertTag = '{cms_module module="groupdocs_viewer_java" width="' . $item->width . '" height="' . $item->height . '" server_url="' . $item->server_url . '"}';
