	...
	)
{
	...
	// FIELDS TO UPDATE
	$query = (isset($item->id)?"UPDATE ":"INSERT INTO ").cms_db_prefix()."module_groupdocs_viewer_java_docs SET 
			width=?,
			height=?,
			server_url=?,
			name=?";
	...
	// VALUES
	$values = array(addslashes($item->width),
		...
		if(isset($item->id)){
			...
			$item->id = $db->GenID(cms_db_prefix()."module_groupdocs_viewer_java_docs_seq");
